...
## CRM Deploy Note
Production URL is `https://www.ducks.co.il`. No `AUTH_URL` env var needed — NextAuth auto-detects on Vercel. `CRM_URL` env var is used for the Payper webhook display in Settings (defaults to `https://www.ducks.co.il` if not set).
